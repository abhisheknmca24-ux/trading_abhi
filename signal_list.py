...
from signal_generator import generate_daily_signals, generate_forced_daily_signals, decide_direction_live
...
def update_signal_list(signal_lines: Optional[List[str]] = None, now: Optional[datetime] = None) -> List[dict]:
    ...
    # Determine if Telegram signals have changed
    needs_rebuild = False
    if signal_lines is not None:
        ...
        if update_id != manager.last_update_id:
            needs_rebuild = True
            manager.last_signal_lines = signal_lines
            manager.last_update_id = update_id
    else:
        # Use last known signals if none provided
        signal_lines = manager.last_signal_lines
            
    # If Telegram signals changed or list is empty (e.g. new day), rebuild the merged list
    if needs_rebuild or not manager.active_signals:
        # 1. Load Telegram signals
        telegram_entries = load_signal_entries(signal_lines or [], current_day)
        telegram_states = _build_signal_state(telegram_entries, source="telegram")

        # 2. Load Generated signals (includes forced if already saved)
        generated_states = load_generated_signals(current_day)

        # 3. Merge and prevent duplicates
        manager.active_signals = _merge_states(telegram_states, generated_states)

        if needs_rebuild or manager.active_signals:
            logger.info(f"Signal list updated (Telegram: {len(telegram_states)}, Generated: {len(generated_states)})")

    # Ensure regular daily signals are generated/fresh
    try:
        generate_daily_signals()
    except Exception as e:
        logger.error(f"Error calling daily generator: {e}")

    # Always ensure forced daily signals (15:05 + 15:10) exist for today
    try:
        forced = generate_forced_daily_signals()
        if forced:
            # Reload generated signals so forced entries appear in active_signals
            generated_states = load_generated_signals(current_day)
            telegram_entries = load_signal_entries(manager.last_signal_lines or [], current_day)
            telegram_states = _build_signal_state(telegram_entries, source="telegram")
            manager.active_signals = _merge_states(telegram_states, generated_states)
    except Exception as e:
        logger.error(f"Error generating forced daily signals: {e}")
...
def _should_take_signal(df: pd.DataFrame, direction: str, confidence: int, is_next_signal: bool) -> tuple[bool, str, int]:
    # 1. Technical Indicator Safety (RSI, Trend, EMAs)
